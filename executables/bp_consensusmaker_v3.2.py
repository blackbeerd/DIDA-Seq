...
import sys
sys.path.append('/home/users/boniface/Python-2.7.2/local/lib/python2.7/site-packages/pysam-0.9.1.2-py2.7-linux-x86_64.egg/')
import pysam
import re
import string
import random
...
def main():
    ...
    parser.add_argument('--tag_type', type=str, action="store", dest='tag_type', default="gd", help="A string specifying which types of read tag to consider.  Read tag types: gd: uses gamma-delta tag and alpha-beta tags in making consensuses (step one of two-step consensus making. ab: alpha-beta tag only (2nd tag in header), removes gamma-delta tags before making consensus (step 2 in 2-step consensus making)")
    o = parser.parse_args()
...
#Start going through the input BAM file, one position at a time.
    for line in bamEntry:
        ...
        while (readWin[winPos%2].pos == readWin[(winPos-1)%2].pos and fileDone==False and readOne==False) or readOne == True:
            ...
            try:
                if 'gd' in o.tag_type:
				#BubblePlex gamma/delta tag
					#Original Loeb lab tag parser:
					#tag = readWin[winPos%2].qname.split('|')[1] + (":1" if readWin[winPos%2].is_read1 == True else (":2" if readWin[winPos%2].is_read2 == True else ":se"))
					tag = string.join(readWin[winPos%2].qname.split('/')[0].split('|')[1:3],'|')  + (":1" if readWin[winPos%2].is_read1 == True else (":2" if readWin[winPos%2].is_read2 == True else ":se"))
					tagDict[tag] += 1 
					#sys.stderr.write(tag[0])
					#print 'tag1:' tag[1]
					#print 'tag2:' tag[2]
					#print 'tag3:' tag[3]
                if 'ab' in o.tag_type:
				#BubblePlex alpha/beta tag
					#First iplimentation of alpha-beta tag:
					...
					#Original Loeb lab tag parser from above used for parsing alpha-beta tags (doesn't seem to work with current tag format- ":GAMMA_DELTA|ALPHA_BETA_TAG"):
					#tag = readWin[winPos%2].qname.split('|')[1] + (":1" if readWin[winPos%2].is_read1 == True else (":2" if readWin[winPos%2].is_read2 == True else ":se"))
					tagDict[tag] += 1
					#print tag
	    except:
                    ...
                    if (consensus.count("N" )/ len(consensus) <= o.Ncutoff and 'n' in o.filt) or ('n' not in o.filt):
                        ...
                        if altTag in consensusDict:
                            if a.is_read1 == True:
                                ...
                                fastqFile1.write('@:%s\n%s\n+\n%s\n' %(consensusDict[altTag].qname, consensusDict[altTag].seq, consensusDict[altTag].qual))
                                outBam.write(a)
